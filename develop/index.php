...
$followers = file_get_contents('http://'.$_SERVER['SERVER_NAME'].'/twitter.php?url=friends&screen_name='.$_REQUEST['screenName']);
...
$elem = array();
foreach($fl->ids as $id){
        if(!empty($id)){
            $el = file_get_contents('http://'.$_SERVER['SERVER_NAME'].'/twitter.php?url=timeline&user_id='.$id.'&count=20&include_entities=true&include_rts=false&exclude_replies=true');
            $e = json_decode($el);
            foreach($e as $it){
                if(!empty($it->errors)){
